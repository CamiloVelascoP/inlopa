...
function crearBuscador(contenedorId, tablaId, datos, columnasBusqueda) {
    console.log(`Creando buscador para ${contenedorId} con ${datos.length} filas de datos`);
    ...
    const buscadorHTML = `
        <div class="buscador-container">
            <div class="buscador-input">
                ...
                <input type="text" id="buscador-${contenedorId}" placeholder="Buscar..." onkeyup="filtrarDatos('${contenedorId}', '${tablaId}', ${JSON.stringify(columnasBusqueda)})">
            </div>
        </div>
    `;
...
function filtrarDatos(contenedorId, tablaId, columnasBusqueda) {
...
// Función genérica para renderizar filas (debe ser sobrescrita por el usuario)
function renderizarFilas(datos, contenedorId) {
    console.warn(`La función renderizarFilas debe ser implementada para ${contenedorId}`);
    return '';
}

// Exportar las funciones para que estén disponibles globalmente
window.crearBuscador = crearBuscador;
window.filtrarDatos = filtrarDatos;
window.renderizarFilas = renderizarFilas; 